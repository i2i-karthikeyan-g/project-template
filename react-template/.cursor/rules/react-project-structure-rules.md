...
alwaysApply: true
---
- Always refer existing code structure and files before creating any thing new 

- For colors and style always refer tailwind.config.js use existing color dont create new color 

-Always Refer existing api,component,styles,state,forms pattern before implementing any new things


1.Code Reuse First
   - Always refer to the existing codebase before creating new components, hooks, utilities, or services.
   - If a similar implementation already exists, extend or reuse it instead of duplicating logic.

2. Tailwind Styling Only
   - Do NOT hardcode hex color values in class names, inline styles, or CSS.
   - Always use color tokens from tailwind.config.js (e.g., bg-primary, text-dark).
   - Do not introduce new custom colors unless added and approved in tailwind.config.js.
...
3. Folder Structure Consistency
   ...
   - Do not create new top-level folders unless necessary and justified.

4. API Call Pattern
   - Use centralized existing api call pattern services under `services/` for all HTTP requests.
   - Do not make direct `axios.get` or `axios.post` calls inside components.
   - Use the existing api call pattern

5. Forms With React Hook Form
   - Use React Hook Form with the existing validation and registration pattern.
   - Reuse any existing form-related components or wrappers if available.
   - Place reusable form fields in `uielements/`.

6. Class-based Local Storage Access
   - Use the class in `storage/LocalStorageService.ts` for all localStorage/sessionStorage access.
   - Do not use React hooks or direct browser APIs for storage.
   - Only use methods like `getItem`, `setItem`, `removeItem` from the provided class.

7. Routing & Layouts
   - Define new routes using the pattern established in `pages/` and `app.tsx`.
   - Use `MainLayout` or other shared layouts for consistency across pages.
...
9. Always Refer react-component-creation-rules.mdc file for creating new components and screens , follow this rules strictly

10. Keep all things closure to each featuer to reduce the touch points for chages 

11. Don't Create barrel export for imports, No index.ts files for all folders use Direct imports allways
